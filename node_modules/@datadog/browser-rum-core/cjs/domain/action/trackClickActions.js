...
function processPointerDown(configuration, lifeCycle, domMutationObservable, pointerDownEvent, windowOpenObservable) {
    ...
    if (configuration.enablePrivacyForActionName) {
        nodePrivacyLevel = (0, privacy_1.getNodePrivacyLevel)(targetForPrivacy, configuration.defaultPrivacyLevel);
    }
    else {
        nodePrivacyLevel = privacyConstants_1.NodePrivacyLevel.ALLOW;
    }
    if (nodePrivacyLevel === privacyConstants_1.NodePrivacyLevel.HIDDEN) {
        return undefined;
    }
...
function startClickAction(configuration, lifeCycle, domMutationObservable, windowOpenObservable, actionTracker, stopObservable, appendClickToClickChain, clickActionBase, startEvent, getUserActivity, hadActivityOnPointerDown) {
    ...
    const pageMayExitSubscription = lifeCycle.subscribe(11 /* LifeCycleEventType.PAGE_MAY_EXIT */, () => {
        click.stop((0, browser_core_1.timeStampNow)());
    });
...
function computeClickActionBase(event, nodePrivacyLevel, configuration) {
    ...
    const selector = (0, getSelectorFromElement_1.getSelectorFromElement)(selectorTarget, configuration.actionNameAttribute);
    if (selector) {
    ...
    const { name, nameSource } = (0, getActionNameFromElement_1.getActionNameFromElement)(nameTarget, configuration, nodePrivacyLevel);
    return {
...
function getEventTarget(event) {
    if (event.composed && (0, htmlDomUtils_1.isNodeShadowHost)(event.target) && typeof event.composedPath === 'function') {
...
function newClick(lifeCycle, actionTracker, getUserActivity, clickActionBase, startEvent) {
    const trackedAction = actionTracker.createTrackedAction((0, browser_core_1.relativeToClocks)(startEvent.timeStamp));
    ...
    function stop(newActivityEndTime) {
        ...
        if (activityEndTime) {
            trackedAction.stop((0, browser_core_1.getRelativeTime)(activityEndTime));
        }
        else {
